...
- AI 코스 우측 상단 플로팅 메뉴 레일 — 동그란 버튼으로 접고 편다
- AI 코스 화면의 물속 연출 — 포인터를 따라 번지는 물방울 파문 캔버스,
  떠오르는 기포, caustic(물결 굴절) 레이어, 포인터를 따르는 물빛.
  기록 사이드바는 햄버거·닫기 버튼으로 여닫는다 (디자인 캔버스 값 그대로)
...
- 지도 — Leaflet 기반 `GeoMap` (fee/days/order/single 4개 모드).
  테마 동선, 장소·축제 위치, 주차장, 저장한 장소, AI 코스 일자별 동선에 쓰인다
- 안내 탭 재작업 — 권역별 9개 섹션(날씨·교통·숙박·주차장·먹거리·축제·
  안내소·무장애·회화)과 스크롤 목차
- 축제 도메인과 상세 화면 `/festivals/[slug]`
- 인증 — 가입·로그인·프로필·알림 설정. 세션 쿠키 방식 (ADR 008)
- 마이페이지 7개 화면. 저장·스탬프는 로그인 없이 브라우저에 보관하고
  GPS로 500m 반경을 판정한다
- AI 코스 탭 — 위저드·프리셋·컴포저·일자별 지도. 키가 없으면 결정적 스텁이
  응답하며 `generated_by`로 구분된다 (ADR 009)
- 홈 — 계절 히어로, 축제 섹션, 후기 카루셀, 야경 마감 섹션
- 데스크톱 푸터 — 브랜드 소개, 약관 링크, AI·날씨 면책 문구, 문의처.
  디자인 캔버스에는 있었으나 구현에서 누락돼 있었다
- 상단 네비게이션에 AI 검색창(문장을 /ai로 넘김)과 로그인·회원가입/로그아웃
  버튼 추가. 디자인 캔버스 대비 빠져 있던 구성 요소다

### Changed

- 안내 탭이 v1/v2 잔존 데이터가 아니라 v3 디자인 스펙을 따른다.
  권역별 관광지 목록은 빠지고 하위 섹션 구조로 바뀌었다
- api 커버리지 하한선 85% -> 95%. greenlet 추적을 켜서 실측이 99%로 올랐다
...
- 후기 요약의 전체 건수·평균을 표시 제한과 분리
- 회원 AI 코스 단건 조회의 소유권 검사, 계정 변경 시 캐시 초기화·사용자별 키 적용
